...
public interface CustomerRepository extends R2dbcRepository<Customer, Long>, CustomerRepositoryInternal {
    ...
    @Query("SELECT * FROM customer entity WHERE entity.address_id = :id")
    Flux<Customer> findByAddress(Long id);

    @Query("SELECT * FROM customer entity WHERE entity.address_id IS NULL")
    Flux<Customer> findAllWhereAddressIsNull();
